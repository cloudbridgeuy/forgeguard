...
impl AllowedOrigin {
    ...
    fn parse(s: &str) -> Result<Self, String> {
        ...
        if let Some(suffix) = s.strip_prefix("*.") {
            if suffix.is_empty() {
                return Err(
                    "suffix origin '*.' must have at least one character after '*.'".into(),
                );
            }
            if !suffix.contains('.') {
                return Err(
                    "suffix origin must have at least two labels (e.g. '*.example.com', not '*.com')"
                        .into(),
                );
            }
        ...
        let Some((_, rest)) = s.split_once("://") else {
            return Err("exact origin must include a scheme (e.g. 'https://example.com')".into());
        };
        if rest.is_empty() {
            return Err("exact origin must contain a host after the scheme".into());
        }
        if rest.contains('/') {
            return Err("exact origin must not contain a path".into());
        }
    ...
    fn matches<'a>(&'a self, origin: &'a str) -> Option<&'a str> {
        match self {
            Self::Exact(expected) => (origin == expected).then_some(expected.as_str()),
            Self::Suffix(suffix) => origin.ends_with(suffix.as_str()).then_some(origin),
            Self::Any => Some("*"),
        }
    }
}
...
impl CorsConfig {
    ...
    /// Returns `true` if the only configured origin is the wildcard `"*"`.
    ///
    /// When wildcard-only, `Vary: Origin` is not needed since the response
    /// is identical for all origins.
    pub fn is_wildcard_only(&self) -> bool {
        matches!(self.origins.as_slice(), [AllowedOrigin::Any])
    }

    /// Push the headers shared by both preflight and normal responses.
    fn push_common_headers(&self, origin: &str, headers: &mut Vec<(String, String)>) {
    ...
    pub fn preflight_headers(&self, origin: &str) -> Vec<(String, String)> {
        ...
        self.push_common_headers(origin, &mut headers);
        headers.push((
...
mod tests {
    ...
    fn parse_exact_missing_scheme_fails() {
        assert!(AllowedOrigin::parse("app.forgeguard.dev").is_err());
    }

    #[test]
    fn parse_exact_with_path_fails() {
        assert!(AllowedOrigin::parse("https://app.dev/api").is_err());
    }
    ...
    fn parse_suffix_single_label_fails() {
        assert!(AllowedOrigin::parse("*.com").is_err());
    }
    ...
    fn exact_matches_same_origin() {
        ...
        assert_eq!(
            o.matches("https://app.forgeguard.dev"),
    ...
    fn preflight_headers_include_all_fields() {
        ...
        assert_eq!(
            header_map["Access-Control-Allow-Origin"],
            "https://app.forgeguard.dev"
        );
    ...
    // -- CorsConfig::is_wildcard_only / Vary ----------------------------------

    #[test]
    fn wildcard_config_is_wildcard_only() {
        let raw = RawCorsConfig {
            allowed_origins: vec!["*".into()],
            ..raw_enabled()
        };
        let config = CorsConfig::try_from(raw).unwrap();
        assert!(config.is_wildcard_only());
    }

    #[test]
    fn exact_config_is_not_wildcard_only() {
        let config = CorsConfig::try_from(raw_enabled()).unwrap();
        assert!(!config.is_wildcard_only());
    }

    #[test]
    fn wildcard_response_has_no_vary() {
        let raw = RawCorsConfig {
            allowed_origins: vec!["*".into()],
            ..raw_enabled()
        };
        let config = CorsConfig::try_from(raw).unwrap();
        let headers = config.response_headers("https://anything.com");
        assert!(!headers.iter().any(|(k, _)| k == "Vary"));
    }

    #[test]
    fn non_wildcard_response_has_vary_origin() {
        let config = CorsConfig::try_from(raw_enabled()).unwrap();
        let headers = config.response_headers("https://app.forgeguard.dev");
        let vary = headers.iter().find(|(k, _)| k == "Vary");
        assert_eq!(vary.unwrap().1, "Origin");
    }

    #[test]
    fn non_wildcard_preflight_has_vary_origin() {
        let config = CorsConfig::try_from(raw_enabled()).unwrap();
        let headers = config.preflight_headers("https://app.forgeguard.dev");
        let vary = headers.iter().find(|(k, _)| k == "Vary");
        assert_eq!(vary.unwrap().1, "Origin");
    ...
    #[test]
    fn wildcard_preflight_has_no_vary() {
        let raw = RawCorsConfig {
            allowed_origins: vec!["*".into()],
            ..raw_enabled()
        };
        let config = CorsConfig::try_from(raw).unwrap();
        let headers = config.preflight_headers("https://anything.com");
        assert!(!headers.iter().any(|(k, _)| k == "Vary"));
    }
}
