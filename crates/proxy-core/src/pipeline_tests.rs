...
use forgeguard_core::{
    FlagConfig, FlagDefinition, FlagName, FlagType, FlagValue, GroupName, OrganizationId,
    PrincipalKind, ProjectId, QualifiedAction, TenantId, UserId,
};
...
impl MembershipResolver for SucceedingMembershipResolver {
    fn resolve(
        &self,
        _user_id: &forgeguard_core::UserId,
        _org_id: &OrganizationId,
    ) -> Pin<Box<dyn Future<Output = Option<Membership>> + Send + '_>> {
        let membership = self.membership.clone();
        Box::pin(async move { Some(membership) })
    }
}

/// A membership resolver that always returns None (not a member).
...
/// Test 1: resolver returns Some — identity is enriched with tenant + groups.
#[tokio::test]
...
/// Test 2: resolver returns None — 403 with correct body.
#[tokio::test]
