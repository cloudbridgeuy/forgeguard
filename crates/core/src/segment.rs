...
/// An `OrganizationId` is validated by the same `Segment` rules as a `TenantId`,
/// so the conversion is infallible — no re-parsing needed.
///
/// Both types are newtypes over [`Segment`] generated by the same macro with
/// identical validation.  Moving via [`Segment`] avoids re-parsing and removes
/// the need for any fallible path.
impl From<OrganizationId> for TenantId {
    fn from(org: OrganizationId) -> Self {
        // Both newtypes wrap a `Segment` — move the inner segment directly.
        Self(org.0)
    }
}
