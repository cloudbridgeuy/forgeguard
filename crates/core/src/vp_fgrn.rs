//! Legacy six-positional FGRN kept solely for the frozen Verified
//! Permissions path. Superseded by [`crate::fgrn::Fgrn`] (Brief v1.4
//! format); deleted when Phase 2 demotes the VP client. Do not add
//! features here.
...
/// `Option<FgrnSegment>::None` at the `VpFgrn` field level. The `-` character
...
///
/// Parse Don't Validate: if you hold an `VpFgrn`, every segment is guaranteed valid.
...
impl VpFgrn {
    ...
    pub fn matches(&self, pattern: &VpFgrn) -> bool {
...
impl fmt::Display for VpFgrn {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(&self.raw)
    }
}

impl FromStr for VpFgrn {
    type Err = Error;
...
impl Serialize for VpFgrn {
    fn serialize<S: Serializer>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error> {
        serializer.serialize_str(&self.raw)
    }
}

impl<'de> Deserialize<'de> for VpFgrn {
...
mod tests {
    ...
    #[test]
    fn parse_user_fgrn() {
        let fgrn = VpFgrn::parse("fgrn:acme-app:acme-corp:iam:user:alice").unwrap();
        assert_eq!(
            fgrn.project(),
    ...
    fn parse_resource_fgrn() {
        let fgrn = VpFgrn::parse("fgrn:acme-app:acme-corp:todo:list:list-001").unwrap();
        assert_eq!(
            fgrn.namespace(),
    ...
    fn parse_wildcard_fgrn() {
        let fgrn = VpFgrn::parse("fgrn:acme-app:*:todo:list:*").unwrap();
    ...
    fn parse_not_applicable_becomes_none() {
        let fgrn = VpFgrn::parse("fgrn:acme-app:-:forgeguard:policy:pol-001").unwrap();
    ...
    fn parse_both_optional_none() {
        let fgrn = VpFgrn::parse("fgrn:-:-:forgeguard:project:acme-app").unwrap();
    ...
    fn wildcard_matches_specific() {
        let specific = VpFgrn::parse("fgrn:acme-app:acme-corp:todo:list:list-001").unwrap();
        let pattern = VpFgrn::parse("fgrn:acme-app:*:todo:list:*").unwrap();
    ...
    fn wrong_namespace_does_not_match() {
        let specific = VpFgrn::parse("fgrn:acme-app:acme-corp:todo:list:list-001").unwrap();
        let pattern = VpFgrn::parse("fgrn:acme-app:acme-corp:billing:list:list-001").unwrap();
        assert!(!specific.matches(&pattern));
    }

    #[test]
    fn none_matches_none() {
        let a = VpFgrn::parse("fgrn:acme-app:-:forgeguard:policy:pol-001").unwrap();
        let b = VpFgrn::parse("fgrn:acme-app:-:forgeguard:policy:pol-001").unwrap();
        assert!(a.matches(&b));
    }

    #[test]
    fn none_pattern_does_not_match_some_value() {
        let value = VpFgrn::parse("fgrn:acme-app:acme-corp:iam:user:alice").unwrap();
        let pattern = VpFgrn::parse("fgrn:acme-app:-:iam:user:alice").unwrap();
        assert!(!value.matches(&pattern));
    }

    #[test]
    fn wildcard_optional_matches_none() {
        let value = VpFgrn::parse("fgrn:acme-app:-:forgeguard:policy:pol-001").unwrap();
        let pattern = VpFgrn::parse("fgrn:acme-app:*:forgeguard:policy:pol-001").unwrap();
        assert!(value.matches(&pattern));
    }
    ...
    fn parse_error_bad_format() {
        assert!(VpFgrn::parse("not-a-fgrn").is_err());
    }

    #[test]
    fn parse_error_too_few_segments() {
        assert!(VpFgrn::parse("fgrn:acme-app:acme-corp:iam:user").is_err());
    }

    #[test]
    fn parse_error_empty() {
        assert!(VpFgrn::parse("").is_err());
    }

    #[test]
    fn parse_error_uppercase_segment() {
        assert!(VpFgrn::parse("fgrn:AcmeApp:acme-corp:iam:user:alice").is_err());
    }
    ...
    fn builder_user() {
        ...
        let fgrn = VpFgrn::user(&project, &tenant, &user_id);
    ...
    fn builder_group() {
        ...
        let fgrn = VpFgrn::group(&project, &tenant, &group);
    ...
    fn builder_policy() {
        ...
        let fgrn = VpFgrn::policy(&project, &policy);
        assert_eq!(
            fgrn.to_string(),
    ...
    fn cedar_entity_type_wildcard_returns_none() {
        let fgrn = VpFgrn::parse("fgrn:acme-app:acme-corp:todo:*:list-001").unwrap();
    ...
    fn serde_round_trip() {
        ...
        let deser: VpFgrn = serde_json::from_str(&json).unwrap();
    ...
    fn new_display_round_trip() {
        ...
        let parsed: VpFgrn = s.parse().unwrap();
    ...
    #[test]
    fn from_str_works() {
        let fgrn: VpFgrn = "fgrn:acme-app:acme-corp:iam:user:alice".parse().unwrap();
        assert_eq!(fgrn.to_string(), "fgrn:acme-app:acme-corp:iam:user:alice");
    }
}
