...
# ForgeGuard Repository Scaffolding — Shaping

## ForgeGuard Context Summary

ForgeGuard is an authorization-as-a-service platform built on AWS (Cognito, Verified Permissions, DynamoDB, S3, SES, SNS). It uses a Smithy-based schema language with custom traits (`@authorize`, `@authResource`, `@featureGate`) and a three-layer design: Design time (Schema), Admin time (AdminClient), Runtime (Guard).
...
**Recommendation for ForgeGuard:** `color-eyre` for binary crates (2 of 3 projects use it, richer debugging for a complex system), `thiserror` for all library crates (unanimous). **Needs confirmation.**
...
**Recommendation for ForgeGuard:** `cargo-rail` (2 of 3 projects, more features including dead feature detection which matters for a workspace with many feature flags). **Needs confirmation.**
...
**Recommendation for ForgeGuard:** Selective via cargo-rail (ForgeGuard will have 8+ crates; running all tests on every change will be slow). **Needs confirmation.**
...
**Recommendation for ForgeGuard:** `tracing` + `tracing-subscriber` (ForgeGuard needs structured logging for event logs, metrics, God Mode, and will benefit from span-based context for auth flows). Despite being used in only 1 project, it's the right choice here. **Needs confirmation.**
...
**Recommendation for ForgeGuard:** Adopt ralph's approach (clippy.toml + source-level denials). The 5-arg threshold and unwrap/expect denials align with ForgeGuard's need for robust, production-quality code. **Needs confirmation.**
...
**Recommendation for ForgeGuard:** Set MSRV to current stable at project creation time. Pin it in `workspace.package.rust-version`. **Needs confirmation.**
...
**Recommendation for ForgeGuard:** calendsync's full config (reviewers, labels, commit message prefixes). ForgeGuard has more dependencies and needs organized dependency updates. **Needs confirmation.**
...
**Recommendation for ForgeGuard:** xtask-only for Rust-based automation. Shell scripts for distribution/installation (install.sh) if needed. Keep hooks in xtask. **Needs confirmation.**

---

## Recommendations for ForgeGuard
...
│   └── dependabot.yml
├── crates/
│   ├── core/                  # forgeguardcore — shared types, traits, error types
│   ├── identity/              # forgeguardidentity — typestate auth flows, event log
│   ├── model/                 # forgeguardmodel — Smithy schema → Verified Permissions
│   ├── control-plane/         # forgeguardcontrol_plane — axum API, dashboard backend
│   ├── sdk/                   # forgeguardsdk — Rust SDK core (Guard, WebhookHandler)
│   ├── ffi/                   # forgeguardffi — PyO3 + WASM bindings
│   ├── cli/                   # forgeguardcli — developer CLI
│   ├── proxy/                 # forgeguardproxy — request interception proxy
│   ├── agent/                 # forgeguardagent — self-hosted data plane agent
...
Existing CI covers single-binary native builds. ForgeGuard needs:
...
Existing pattern: inline `#[cfg(test)]` modules. ForgeGuard additionally needs:
...
Existing: tag-based GitHub releases with binary artifacts. ForgeGuard needs:
...
| D4  | CI change detection          | `cargo-rail` selective                                                                                                         | ForgeGuard has 8+ crates, full suite too slow               |
...
| D12 | Crate naming                 | `forgeguard` prefix + terse industry suffix (authn, authz, audit)                                                              | Short names compound across codebase                        |
...
| D23 | docker-compose.yml           | Yes — for LocalStack (dev/CI)                                                                                                  | New pattern for ForgeGuard, not in existing projects        |
