# xtask lint Overhaul — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use executing-plans to implement this plan task-by-task.

**Goal:** Replace the current ad-hoc `xtask lint` with a configuration-driven pipeline adapted from mcptools, producing zero output on success and error output + exit code 1 on failure.

**Architecture:** Functional Core (pure types, skip logic, outcome determination, log formatting) / Imperative Shell (process execution, file I/O, orchestration). The check pipeline is defined as a static `CHECKS` array — adding or removing a check is a one-line change. File-length validation is a custom check unique to forgeguard.

**Tech Stack:** `clap` (CLI), `duct` (child processes), `color-eyre` (errors), `glob` (file scanning), `chrono` (hook backups), `cargo-rail` (dependency unification)

---

## Task 1 — Add `duct` dependency to xtask
...
This drops `thiserror`, `tokio`, `anstream`, `serde`, `serde_json` — none are used by lint. Adds `duct` for child process piping.

**Verify:** `cargo check -p xtask` compiles.

---

## Task 2 — Promote `lint.rs` to directory module

**Actions:**

1. Create directory `xtask/src/lint/`
2. Move `xtask/src/lint.rs` → `xtask/src/lint/mod.rs` (will be overwritten in Task 3)
3. Create empty `xtask/src/lint/hooks.rs`

**Verify:** `cargo check -p xtask` still compiles (contents unchanged, just moved).

---

## Task 3 — Write the lint pipeline (Functional Core + Imperative Shell)

**File:** `xtask/src/lint/mod.rs`
...
fn fix_args(id: CheckId) -> Option<Vec<&'static str>> {
    match id {
        ...
        CheckId::Clippy => Some(vec![
            "clippy", "--workspace", "--all-targets", "--fix", "--allow-dirty", "--", "-D", "warnings",
        ]),
...
/// Validate file lengths. Returns `CheckResult` with pass/fail.
///
/// Pure-ish: takes the file list as input so globbing can be tested separately.
...
/// Execute a single cargo check and return its result.
...
mod tests {
    ...
    fn test_is_tool_not_found_detects_missing_tool() {
        ...
        assert!(is_tool_not_found("error[E0463]: No such file or directory\n"));
        assert!(is_tool_not_found("error: unrecognized subcommand 'machete'\n"));
    }
    ...
    fn test_format_log_entry_passed() {
        let result = CheckResult {
            ...
            outcome: CheckOutcome::Passed { output: "all good\n".to_string() },
        };
    ...
    fn test_format_log_entry_failed() {
        let result = CheckResult {
            ...
            outcome: CheckOutcome::Failed { output: "error: something wrong\n".to_string() },
        };
        let entry = format_log_entry(&result);
    ...
    fn test_determine_outcome_optional_but_real_failure() {
        let outcome = determine_outcome(false, "error[E0308]: mismatched types\n".to_string(), true);
        assert!(matches!(outcome, CheckOutcome::Failed { .. }));
    }
    ...
    fn test_file_lengths_all_under_limit() {
        ...
        assert!(matches!(evaluate_file_lengths(&files, 1000), CheckOutcome::Passed { .. }));
    }
    ...
    fn test_file_lengths_empty_list() {
        assert!(matches!(evaluate_file_lengths(&[], 1000), CheckOutcome::Passed { .. }));
    }
...
- `FileLength` is in the `CHECKS` array with a sentinel `program: "__builtin__"` — the shell checks `check.id == CheckId::FileLength` and runs `collect_file_lengths()` + `evaluate_file_lengths()` instead of spawning a process. The pure `evaluate_file_lengths` function is fully unit-testable.
- `Rail` runs `cargo rail unify --check` (dependency unification, dead features, unused deps, MSRV). Marked `optional: true` so it's gracefully skipped if `cargo-rail` isn't installed. In `--fix` mode it drops `--check` to apply fixes.
- Zero output on success (no log path printed). On failure: error output + `lint failed at:` + log path + `exit(1)`.
- `--workspace` on check/clippy/test to match forgeguard convention.

**Verify:** `cargo test -p xtask` — all unit tests pass.

---

## Task 4 — Write hooks management

**File:** `xtask/src/lint/hooks.rs`
...
No changes needed from current — just update the doc comment on `Lint` variant.

**Verify:** `cargo build -p xtask`

---

## Task 6 — Run the full lint pipeline

Run `cargo xtask lint` and fix any issues it reports.

**Expected on success:** zero output, exit code 0.
**Expected on failure:** error output from the failing check, `lint failed at: <name>`, log path, exit code 1.

---

## Task 7 — Run xtask unit tests

Run `cargo test -p xtask` and confirm all tests pass.
