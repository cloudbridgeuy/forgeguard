...
Reads `xtask/seed.toml`, tears down prior fixtures, and re-seeds each declared org as `OrgStatus::Draft` with its user schema, RBAC roles, and Cognito users. **No Verified Permissions policies are pushed** — the V3 Active-org VP write-path is owned by the saga ticket (separate from issue #102).

The command runs in three phases:

1. **Phase 0 (preflight, pure)** — `pure::preflight_validate` walks every declared org and, before any AWS call, validates `OrganizationId::new(org_id)`, `PoolId::try_new(cognito_user_pool_id)`, converts each `[organization.user_schema]` into the domain `UserSchema`, runs `validate_attributes` against every declared user's attributes, validates each user's `GroupName`s, and validates the group inheritance graph (name regex, dangling inherit, cycles). A failure anywhere aborts the seed before Teardown or Re-seed touch AWS. On success it returns one `UserSchema` per org (index-aligned with `orgs`), which Phase 2 and Phase 4 reuse instead of re-parsing `seed.toml`.
