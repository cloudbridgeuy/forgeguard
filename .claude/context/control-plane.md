...
V3 replaces the V2 `todo!("V3")` Active branch with the full distributed-write logic: pure pre-flight → DDB write → VP parent push (delete-then-create on UPDATE) → deterministic alphabetical-order fanout to inheriting dependents. The new `vp_client` module (trait + `AwsVpClient` + `StubVpClient`) lives in `crates/control-plane/src/vp_client/`; failure modes F3 (rollback succeeds → 503), F3' (rollback fails → 500 + `forgeguard_cp_group_rollback_failed_total{stage="parent"}` increment), and F4 (mid-fanout fail → 503 with `{completed, failed, remaining}` shape, no rollback) are covered by integration tests in `crates/control-plane/src/handlers/tests/groups_active_*.rs`. **No real Active org exists yet** — the saga that flips `Draft → Active` and seeds the per-org VP store is V4. V3 is exercised end-to-end via the stub today; the production hot path stays Draft until the saga ships. Implementation plan: [`.claude/designs/issue-102-v3-implementation-plan.md`](../designs/issue-102-v3-implementation-plan.md). Manual QA plan: [`.claude/plans/issue-102-v3-implementation-plan-qa.md`](../plans/issue-102-v3-implementation-plan-qa.md).
