...
- **CP authz (#117 V1 embedded):** `cp:*` decisions run in-process on `CpCedarEngine`, compiled from `forgeguard.toml` at build time via `crates/control-plane/build.rs` — zero AWS Verified Permissions calls on the request path; `DefaultPolicy::Deny` when `--jwks-url` is set; `--policy-store-id` still parsed but ignored — see [control-plane.md](./.claude/context/control-plane.md)
