...
- **Control plane:** Axum service, `--store=memory` (dev) or `--store=dynamodb` (prod). `OrgStore` is object-safe via `#[async_trait]`; runtime carries `Arc<dyn OrgStore>`. Handlers take `State<Arc<dyn OrgStore>>` directly, or `State<AppState>` (non-generic since #117) when they also need `state.model_events` — never `<S: OrgStore>` — see [control-plane.md](./.claude/context/control-plane.md)
- **Optimistic locking:** write-side `If-Match` is gone — org `PUT` and group `PUT`/`DELETE` both use revision tokens (`X-Fg-If-Revision` / `X-Fg-Revision`, 412 `revision_mismatch`); conditional GET (`If-None-Match` / 304) survives read-side on org config and group reads — see [control-plane.md](./.claude/context/control-plane.md) and [groups-v3.md](./.claude/context/groups-v3.md)
...
- **CP authz (#117 embedded):** `cp:*` decisions run in-process on `CpCedarEngine`, compiled from `forgeguard.toml` at build time via `crates/control-plane/build.rs` — zero AWS Verified Permissions calls on the request path; `DefaultPolicy::Deny` when `--jwks-url` is set; auth flags are `--jwks-url`/`--issuer`/`--audience` only (`--policy-store-id` deleted in #117 V3) — see [control-plane.md](./.claude/context/control-plane.md)
...
- **Groups CRUD (#117 V2 event-sourced append):** group PUT/POST/DELETE is a pure event-sourced append (`ModelEventStore::{put_group,delete_group}`) for every org status — no VP push, no F-VP/F-VP-mid/F-append failure modes, no rollback metric; `X-Fg-If-Revision` replaces ETag/If-Match on group PUT/DELETE (412 `revision_mismatch` unchanged) — see [groups-v3.md](./.claude/context/groups-v3.md)
...
- **Membership model:** JWT is identity-only (`sub`); org + groups resolved per-request from `X-Fg-Org-Id` header + DynamoDB `PK=USER#{sub}, SK=ORG#{org_id}` lookup (pipeline Phase 5b). Inverted GSI1 lists users per org — see [authn-wiring.md](./.claude/context/authn-wiring.md) and [control-plane.md](./.claude/context/control-plane.md)
...
- **CP Lambda runtime contract:** the CP function needs `FORGEGUARD_CP_{JWKS_URL,ISSUER,AUDIENCE,COGNITO_POOL_ID}` + `TABLE_NAME` env vars + a DynamoDB-only IAM grant — no Verified Permissions IAM anywhere (the CDK `VerifiedPermissionsStack` and both VP IAM grants were deleted in #117 V3). See [infra-control-plane.md § Control-plane Lambda runtime contract](./.claude/context/infra-control-plane.md#control-plane-lambda-runtime-contract)
...
- **Seed QA playbook:** end-to-end validation scenarios for `cargo xtask control-plane seed` (lint, unit, DDB integration, loader, local e2e) — see [seed-qa.md](./.claude/context/seed-qa.md) and the V5 plan at `.claude/plans/2026-05-06-issue-102-cp-groups-v5/v5-plan-qa.md`
...
- **xtask deps** — `xtask` is a binary that intentionally minimizes workspace path deps to keep the cached binary fresh. **Exception:** pure leaf crates (currently `crates/authz-core`) may be consumed by `xtask` to share canonical types and pure functions across consumers (e.g. the RBAC compiler used by both `cargo xtask control-plane seed`'s validation and the V2+ control-plane Groups handlers). Keep this list small and pure-only.
