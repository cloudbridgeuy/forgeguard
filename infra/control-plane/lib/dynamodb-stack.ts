...
export class DynamoDbStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: DynamoDbStackProps) {
    ...
    // they stay out of it entirely, instead of a full-table PK/SK inversion
    // where every item would be indexed.
    //
    // Changing this GSI's KeySchema, or changing more than one replica,
    // requires separate deploys — CloudFormation rejects both an in-place
    // GSI KeySchema/Projection change and removing/adding more than one
    // replica in a single update.
