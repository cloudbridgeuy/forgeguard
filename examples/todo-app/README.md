...
- `bun` (for CDK deployments — see [infra/dev/README.md](../../infra/dev/README.md))

## Setup Guide

Both paths require Verified Permissions (authorization). The difference is
whether you also deploy Cognito (authentication via JWT).

### Path 1 — API keys only (no Cognito)

Use this if you want to skip JWT auth and use static API keys exclusively.
All demo scenarios work except JWT-based authentication.

1. Deploy the VP policy store:

   ```bash
   cargo xtask dev setup --vp
   ```

   This creates `forgeguard.dev.toml` with the VP `policy_store_id` filled in.
   See [infra/dev/README.md](../../infra/dev/README.md) for prerequisites and
   details.

2. Edit `forgeguard.dev.toml` and set the auth chain to API keys only:
   ...
   Leave the `[authn.jwt]` section commented out or remove it.

3. Run the demo (see [Quick Start](#quick-start) below), using
   `--config forgeguard.dev.toml`.

### Path 2 — Full stack (JWT + API keys + VP)

Use this for the complete experience including Cognito JWT authentication.

1. Deploy both Cognito and VP:

   ```bash
   cargo xtask dev setup --all
   ```

   This creates `forgeguard.dev.toml` with Cognito JWKS/issuer and VP
   `policy_store_id` filled in. See
   [infra/dev/README.md](../../infra/dev/README.md) for prerequisites and
   details.
...
4. To test with a JWT instead of an API key:
...
> **Note:** The `examples/todo-app/forgeguard.toml` file is a reference
> template with AWS sections commented out. For running the demo, always use
> `forgeguard.dev.toml` (generated by `cargo xtask dev setup`).
...
In `forgeguard.dev.toml` this is `127.0.0.1:9090`; in the Docker Compose setup
it's `0.0.0.0:6150`.
